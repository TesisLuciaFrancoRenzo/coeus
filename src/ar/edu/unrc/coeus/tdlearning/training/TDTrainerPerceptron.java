...
public class TDTrainerPerceptron extends Trainer {
        ...
        for ( int layerIndex = 0; layerIndex < neuralNetwork.getLayerQuantity(); layerIndex++ ) {
            ...
            for ( int neuronIndex = 0; neuronIndex < neuronQuantityInLayer; neuronIndex++ ) {
                if ( layerIndex != 0 ) {
                    ...
                    for ( int previousNeuronIndex = 0; previousNeuronIndex < neuronQuantityInPreviousLayer;
                            previousNeuronIndex++ ) {
                        for ( int outputNeuronIndex = 0; outputNeuronIndex < outputLayerNeuronQuantity;
                                outputNeuronIndex++ ) {
                            elegibilityTraces.get(layerIndex).get(neuronIndex).get(previousNeuronIndex).set(
    ...
    public void train(
        ...
        if ( firstTurn ) {
            ...
            final int neuronQuantityAtOutput = turnCurrentStateCache.getLayer(turnCurrentStateCache.
                    getOutputLayerIndex()).getNeurons().size();
            if ( tDError == null ) {
        ...
        for ( int i = 0; i < this.nextTurnOutputs.size(); i++ ) {
            this.nextTurnOutputs.set(i, nextTurnStateCache.getLayer(nextTurnStateCache.getOutputLayerIndex()).getNeuron(
                    i).getOutput());
        }
            ...
            layerJStream.forEach(neuronIndexJ ->
                    {
                        ...
                        layerKStream.forEach(neuronIndexK ->
                                {
                                    final double oldWeight = currentNeuron.getWeight(
                                            neuronIndexK);
                                    if ( !isARandomMove || nextTurnState.
                                        ...
                                        final double newDiferential = computeWeightError(
    ...
    private Double calculateNeuronOutput(
        ...
        if ( neuronIndex == currentLayer.getNeurons().size() && turnCurrentStateCache.getOutputLayerIndex() != layerIndex && neuralNetwork.
                hasBias(nextLayer) ) {
            //retorno la salida de la neurona falsa
            return 1d; //FIXME y si no tiene bias??? TESTEAR CAMBIOS
        } else {
    ...
    private void calculateTDError(final IStatePerceptron nextTurnState) {
        final Layer outputLayerCurrentState = turnCurrentStateCache.getLayer(
                turnCurrentStateCache.getOutputLayerIndex());
        final int neuronQuantityAtOutput = outputLayerCurrentState.getNeurons().size();
    ...
    private Double computeEligibilityTrace(
        ...
        final double derivatedOutput = delta(outputNeuronIndex, layerIndexJ,
                neuronIndexJ) * calculateNeuronOutput(layerIndexK, neuronIndexK);
        if ( this.lambda > 0 ) {
            final List<Double> neuronKEligibilityTrace = elegibilityTraces.get(
            ...
            if ( isRandomMove && replaceEligibilityTraces ) {
                ...
                final double newEligibilityTrace = neuronKEligibilityTrace.get(
                ...
                final double newEligibilityTrace = (neuronKEligibilityTrace.get(outputNeuronIndex) * lambda * gamma) + derivatedOutput; //reutilizamos las viejas trazas
                neuronKEligibilityTrace.set(outputNeuronIndex, newEligibilityTrace);
                return newEligibilityTrace;
            }
        } else {
    ...
    private Double computeWeightError(
        ...
        final int outputLayerSize = turnCurrentStateCache.getLayer(
            ...
            return lastLayerStream.mapToDouble(outputNeuronIndex ->
                    {
                        return alpha[layerIndexJ] * tDError.get(outputNeuronIndex)
                                * computeEligibilityTrace(outputNeuronIndex, layerIndexJ, neuronIndexJ, layerIndexK,
                ...
                .forEach(l ->
                        {
                            ...
                            IntStream currentLayerStream = IntStream.range(0, neuralNetwork.getNeuronQuantityInLayer(
                                    currentLayerIndex));
                            ...
                            currentLayerStream.forEach(currentNeuronIndex ->
                                    {
                                        ...
                                        if ( currentLayerIndex == 0 ) {
                                            ...
                                            if ( neuralNetwork.hasBias(
                                                    currentLayerIndex) ) {
                                                neuron.setBias(neuralNetwork.getBias(currentLayerIndex,
                                                        currentNeuronIndex));
                                            }
                                            ...
                                            IntStream previousLayerStream = IntStream.
                                                    range(0, neuralNetwork.getNeuronQuantityInLayer(previousLayerIndex));
                                            ...
                                            neuron.setOutput(neuralNetwork.getActivationFunction(currentLayerIndex).
                                                    apply(net));
                                            neuron.setDerivatedOutput(
                                                    neuralNetwork.getDerivatedActivationFunction(currentLayerIndex).
